...
public class Planner {
	...
	public void execute_goalstack_planner(){
		...
		while(!success){
			...
			if(goal_stack.getLast() instanceof Predicate){
				...
				if(this.curr_state.getPredicates().contains(goal_stack.getLast())){
					goal_stack.removeLast();
				}
				else if(p instanceof Adjacent){
					Adjacent pAux = (Adjacent) p;
					if(pAux.getO1()==null){
						//find best adjacent empty
						pAux.setO1(best_adjacentEmpty(pAux.getO2()));
						propagate(pAux.getO1());
					}else if(pAux.getO2()==null){
						pAux.setO2(best_adjacentEmpty(pAux.getO1()));
						propagate(pAux.getO2());
					}
					if(pAux.getO1().is_adjacent(pAux.getO2())){
						goal_stack.removeLast();
					}else{
						System.out.println("ERROR FOUND ON ADJACENCY CONDITION"+pAux.toString());
					}
				}
			...
			if(goal_stack.isEmpty()){
				success = true;
				System.out.println(this.operator_solution);
			}
		}
			...
			if(p1.getOffice()!=null){
				if(p1.getOffice().is_adjacent(this.curr_state.getRobotPosition())){
					...
					if(this.curr_state.robotOnBox()){
						//if on box and the objective is to move to a place where the box must be
						if(this.goal_state.getOfficeBox(this.curr_state.getRobotPosition().id)!=null && this.goal_state.getOfficeBox(this.curr_state.getRobotPosition().id).equals(p1.getOffice()))
							return new Push(this.curr_state.getOfficeBox(this.curr_state.getRobotPosition().id),this.curr_state.getRobotPosition(),p1.getOffice());
						else{
							//no push necessary
					...
					// suposarem un move desde adjacent
					if(p1.getOffice()==null)
						System.out.println("Error office");
					else if(this.curr_state.getRobotPosition()==null)
						System.out.println("No robot position");
					Office of1=this.best_adjacent(p1.getOffice(), this.curr_state.getRobotPosition());
					return new Move(of1,p1.getOffice());
				}
			}else{
	...
	public Office best_adjacent(Office o1, Office o2){
		...
		return randOffice(this.curr_state.getAdjacentOffices());
	}

	public Office best_adjacentEmpty(Office o1){
		for(Office of1:this.curr_state.getAdjacentOffices(o1)){
			if(of1.is_adjacent(o1)&&this.curr_state.isEmpty(of1))
				return of1;
	...
	public void propagate(Object p1){
		if(p1 instanceof Office){
			for(int i=this.goal_stack.size()-1;i>=0;i--){
				Object o = this.goal_stack.get(i);
				// case operator
				if(o instanceof Operator){
					Operator o1 = (Operator) o;
					if(o1.isPartial()){
						if(o1 instanceof CleanOffice){
							CleanOffice co = (CleanOffice) o1;
							if(co.getO()==null)
								this.goal_stack.set(i, new CleanOffice((Office)p1));
						}else if(o1 instanceof Move){
							Move m = (Move)o1;
							if(m.getO1()==null){
								this.goal_stack.set(i, new Move((Office)p1, m.getO2()));
							} else if (m.getO2()==null){
								this.goal_stack.set(i,new Move(m.getO1(),(Office)p1));
							}
						}else if(o1 instanceof Push){
							Push m = (Push)o1;
							if(m.getO1()==null){
								this.goal_stack.set(i, new Push(m.getB(),(Office)p1, m.getO2()));
							} else if (m.getO2()==null){
								this.goal_stack.set(i,new Push(m.getB(),m.getO1(),(Office)p1));
							}
						}
					}
					break;
					...
					Predicate o1 = (Predicate) o;
					if(o1 instanceof Adjacent){
						//Adjacent p1 = (Adjacent) o;

					}else if(o1 instanceof BoxLocation){
						BoxLocation p2 = (BoxLocation) o;
						if(p2.getOffice()==null)
							this.goal_stack.set(i, new BoxLocation(p2.getBox(),(Office)p1));
					}else if(o1 instanceof Clean){
						Clean c = (Clean) o1;
						if(c.getOffice()==null)
							this.goal_stack.set(i, new Clean((Office)p1));
					}else if(o1 instanceof Dirty){
						Dirty d = (Dirty) o1;
						if(d.getOffice()==null)
							this.goal_stack.set(i, new Dirty((Office)p1));
					}else if(o1 instanceof Empty){
						Empty e = (Empty) o1;
						if(e.getO()==null)
							this.goal_stack.set(i, new Empty((Office)p1));
					}else if(o1 instanceof RobotLocation){
						RobotLocation rl = (RobotLocation) o1;
						if(rl.getOffice()==null)
							this.goal_stack.set(i, new RobotLocation((Office)p1));
					}
					//case arraylist
				}else if(o instanceof ArrayList){
					ArrayList<Predicate> plist = (ArrayList<Predicate>) o;
					for(int j=0;j<plist.size();j++){
						Predicate o1 = (Predicate) plist.get(j);
						if(o1 instanceof Adjacent){
							//Adjacent p1 = (Adjacent) o;
						}else if(o1 instanceof BoxLocation){
							BoxLocation p2 = (BoxLocation) o1;
							if(p2.getOffice()==null)
								plist.set(j, new BoxLocation(p2.getBox(),(Office)p1));
						}else if(o1 instanceof Clean){
							Clean c = (Clean) o1;
							if(c.getOffice()==null)
								plist.set(j, new Clean((Office)p1));
						}else if(o1 instanceof Dirty){
							Dirty d = (Dirty) o1;
							if(d.getOffice()==null)
								plist.set(j, new Dirty((Office)p1));
						}else if(o1 instanceof Empty){
							Empty e = (Empty)o1;
							if(e.getO()==null)
								plist.set(j, new Empty((Office)p1));
						}else if(o1 instanceof RobotLocation){
							RobotLocation rl = (RobotLocation) o1;
							if(rl.getOffice()==null)
								plist.set(j, new RobotLocation((Office)p1));
						}
					}
					//plist has the list fixed
					this.goal_stack.set(i, plist);
				}
			}
		} else if(p1 instanceof Box){
				...
				if(o instanceof Operator){
					...
					if(o1.isPartial()){
						if(o1 instanceof CleanOffice){
							...
							Push m = (Push)o1;
							if(m.getB()==null){
								this.goal_stack.set(i, new Push((Box)p1,m.getO1(), m.getO2()));
							}
						}
					}
					break;
					...
					if(o1 instanceof Adjacent){
						...
						BoxLocation p2 = (BoxLocation) o;
						if(p2.getBox()==null)
							this.goal_stack.set(i, new BoxLocation((Box)p1,p2.getOffice()));
					}else if(o1 instanceof Clean){
					}else if(o1 instanceof Dirty){
					}else if(o1 instanceof Empty){
					}else if(o1 instanceof RobotLocation){
					...
					for(int j=0;i<plist.size();j++){
						...
						if(o1 instanceof Adjacent){
							...
							BoxLocation p2 = (BoxLocation) o;
							if(p2.getBox()==null)
								plist.set(j, new BoxLocation((Box)p1,p2.getOffice()));
						}else if(o1 instanceof Clean){
						}else if(o1 instanceof Dirty){
						}else if(o1 instanceof Empty){
						}else if(o1 instanceof RobotLocation){
	 ...
	 * @return
	 */
	public Office randOffice(ArrayList<Office> array) {
